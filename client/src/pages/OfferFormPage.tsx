...
import { listCourses, getOffer, createOffer, updateOffer, type Course } from "../lib/api";
...
export default function OfferFormPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      created_at:   formatBRToISO(createdBR) || new Date().toISOString(),
    };
  ...
  return (
    <section className="container mx-auto max-w-2xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold">{editing ? "Editar oferta" : "Nova oferta"}</h1>
      ...
      <form className="space-y-4" onSubmit={onSubmit}>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label className="block text-sm font-medium">Início (DD/MM/AAAA)</label>
            <input className="input" placeholder="DD/MM/AAAA" value={startBR} onChange={e=>setStartBR(e.target.value)} required/>
          </div>
          <div>
            <label className="block text-sm font-medium">Fim (DD/MM/AAAA)</label>
            <input className="input" placeholder="DD/MM/AAAA" value={endBR} onChange={e=>setEndBR(e.target.value)} required/>
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium">Criado em (DD/MM/AAAA) – opcional</label>
          <input className="input" placeholder="DD/MM/AAAA" value={createdBR} onChange={e=>setCreatedBR(e.target.value)}/>
        </div>
